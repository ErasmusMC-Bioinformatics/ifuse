    ...
    */
    public function organize(&$data) {
	    ...
	    if (is_array($url) && is_array($data) && (sizeof($url) > 0)) {
            ...
            if (!$this->sorted) {
                $return[0] = array(); # fusions
                $return[1] = array(); # rest
                $done = false;
                foreach ($data as &$element) {
                    if ((!is_null($element['left.donor'])) ||(!is_null($element['right.donor']))) { # is a fusiongene
                        $return[0][] =& $element;
                    } else { # is not a fusiongene
                        $return[1][] =& $element;
                    }
               }
               $data = array_merge($return[0], $return[1]);
            }
            $this->organized = true;
        }
